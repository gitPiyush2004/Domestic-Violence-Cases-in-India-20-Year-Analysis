...
def main() -> None:
    ...
    # Written to BOTH sites. dashboard/ and powerbi-showcase/ deploy as separate
    # Netlify sites, each with its own folder as the web root, so the showcase
    # cannot reach ../dashboard/data.js at runtime - that path escapes its root
    # and 404s, leaving the whole report blank. Emitting the file twice from the
    # one generator keeps the copies identical by construction; a hand-copied
    # duplicate would drift the first time the ETL is re-run.
    targets = [ROOT / "dashboard" / "data.js",
               ROOT / "powerbi-showcase" / "data.js"]
    for t in targets:
    ...
    size_kb = len(js) / 1024
    for t in targets:
        print(f"Wrote {t.relative_to(ROOT).as_posix()}  ({size_kb:.0f} KB)")
