...
The report is built: **five pages, 40 visuals, 24 of them also laid out for phone**,
generated by `powerbi/generate_report.ps1` and verified to open in Desktop.

Everything the README describes now exists in the repository. What a reviewer should
know before opening it: the visuals are generated from a compact layout declaration
rather than hand-drawn, so to move something, edit the declaration and re-run the
script — the JSON under `DomesticViolence.Report/definition/pages/` is a build
artefact and hand-edits will be overwritten.

Not done, and worth saying plainly: the report has been verified to **load**, not to
look finished. Colours come from the stock theme rather than the palette in
`docs/DASHBOARD_GUIDE.md`, and no one has yet sat with each page to tune axis
formatting, data labels and legend placement. That is a pass with Desktop open, not a
rebuild.
